...
def log(message):
    print("%s: %s" % (get_formatted_time(), message), flush=True)
...
def wait_until_package_installed(base_url, credentials, package_reference):
    ...
    if package_reference.find('aem-service-pkg-6.2.SP1') > -1:
        ...
        f = subprocess.Popen(['tail', '-F', '/opt/aem/crx-quickstart/logs/error.log'], stdout=subprocess.PIPE,
        ...
        while not installed:
            ...
            try:
                ...
                log("Package \"%s\" not yet installed. Curl error: %s. Will retry in 10 seconds..." % (
                package_reference, error))
                sleep(10)
                continue

            if not is_json(package_installation_response):
                log("Package \"%s\" not yet installed. Package installation check response: %s. Will retry in 10 seconds..." % (
                package_reference, package_installation_response))
                sleep(10)
                continue
            ...
            for result in json_response["results"]:
                ...
                package_reference_response = "%s-%s" % (result["name"], result["version"]) if len(
                    result["version"]) > 0 else result["name"]
...
def import_packages(aem_jar_file_name, port, run_mode, username='admin', password='admin', package_dir='packages'):
    log("******************* Start installing packages *******************")

    base_url = "http://localhost:" + port
    credentials = "%s:%s" % (username, password)
    current_dir = os.getcwd()
    author_mode = True if run_mode.find('author') > -1 else False

    server_process_id = start_aem_server(aem_jar_file_name, port, run_mode)
    ...
    for package_file_name in sorted(os.listdir(os.path.join(current_dir, package_dir))):
        ...
        sleep(300)
        server_process_id = restart_aem_server(server_process_id, aem_jar_file_name, port, run_mode)
...
def run_compaction(oak_path, aem_quickstart_folder):
    ...
    return_code = subprocess.call(
        ['java', '-Dtar.memoryMapped=true', '-Xms8g', '-Xmx8g', '-jar', oak_path, 'checkpoints',
         aem_quickstart_folder + '/repository/segmentstore'])
    log('Return code of finding old AEM checkpoints process: %s' % return_code)

    log('Deleting unreferenced AEM checkpoints...')
    return_code = subprocess.call(
        ['java', '-Dtar.memoryMapped=true', '-Xms8g', '-Xmx8g', '-jar', oak_path, 'checkpoints',
         aem_quickstart_folder + '/repository/segmentstore', 'rm-unreferenced'])
    ...
    return_code = subprocess.call(['java', '-Dtar.memoryMapped=true', '-Xms8g', '-Xmx8g', '-jar', oak_path, 'compact',
                                   aem_quickstart_folder + '/repository/segmentstore'])
...
def start_aem_server(aem_jar_file_name, port, run_mode):
    arguments = "file: %s, port: %s, run mode: %s" % (aem_jar_file_name, port, run_mode)
    ...
    install_process = subprocess.Popen(['java', '-Xms8g', '-Xmx8g', '-Djava.awt.headless=true',
                                        '-jar', aem_jar_file_name, '-listener-port', str(LISTENER_PORT), '-r', run_mode,
                                        '-p', port, '-nofork'])
    ...
    while True:
        ...
        try:
            socket_instance.bind((HOST, LISTENER_PORT))
            log("Succeeded biding on socket")
            break
    ...
    log("Listen done..." + str(address))

    str_result = ""
    while True:
        log("Trying to start the AEM after listener...")
        data = socket_connection.recv(1024)
        if not data:
            log("Failed starting AEM: %s" % arguments)
            log("Failed starting AEM received:  %s" % str_result)
            install_process.kill()
            sys.exit(1)
        else:
            str_result = str_result + str(data.decode('utf8')).strip()
...
def restart_aem_server(process_id, aem_jar_file_name, port, run_mode):
    log("Restarting AEM...")
    stop_aem_server(process_id)
    new_pid = start_aem_server(aem_jar_file_name, port, run_mode)
...
def update_author_replication_agent(base_url, credentials="admin:admin"):
    ...
    c.perform()
    c.close()
    agent_status_response = agent_status.getvalue().decode('UTF-8')
    agent_status.close()
